...
- Never derive a field key, read a `TSH_UI_LOGIN_*` variable, or `fill` a credential on a page whose post-redirect origin (scheme, host, port) differs from the session's authorized origin — the origin recorded once from the caller-provided user-confirmed URL. Stop before all three, return a blocker naming the observed origin, and wait for the caller to relay the user's explicit authorization of that origin for the session; that authorization is the only route to a cross-origin login page and is how SSO, MFA, and captcha screens on another origin stay reachable. Never authorize an origin yourself, and never widen the read: only the variables you requested for the form on screen may be read, because the `TSH_UI_LOGIN_` prefix is a naming convention, not an allowlist.
...
- the session's authorized origin, plus the observed post-redirect page origin whenever it differs from it
