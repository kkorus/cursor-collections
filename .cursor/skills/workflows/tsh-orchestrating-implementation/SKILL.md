...
| --- | --- | --- | --- |
| app code (plan task) | `tsh-plan-implementor` | `.cursor/skills/internal/tsh-implement-common-task/SKILL.md` | DEFAULT route in both Quick Flow and Full Flow for approved, actionable, low-risk plan seams that must be executed exactly as written |
| app code (complex or no-plan) | `tsh-software-engineer` | `.cursor/skills/internal/tsh-implement-common-task/SKILL.md` | EXCEPTION route for complex non-UI work, or for no-plan non-UI execution after the no-plan confirmation gate; choose `GPT-5.3-Codex` for medium-reasoning precision on complex work, or `Gemini 3.5 Flash` for fast, low-cost, large-context analysis |
| UI with Figma | `tsh-ui-engineer` | `.cursor/skills/internal/tsh-implement-ui-common-task/SKILL.md` | The internal skill should be used for Figma-based UI implementation |
...
6. **Handle `[REUSE]` UI verification as a per-item loop:**
   ...
   - Then delegate the same item to `tsh-ui-reviewer` following `.cursor/skills/commands/tsh-review-ui/SKILL.md`, passing the Figma URL, the same pinned full dev server URL unchanged, the component or section name, and the exact artifact directory produced by `tsh-ui-capture-worker`.
   ...
   - When the 5-iteration budget is exhausted with remaining gaps, route through the structured user-confirmation gate defined in `.cursor/skills/internal/tsh-implement-ui/SKILL.md` rather than improvising a local escalation path.
...
8. **Run code review after the UI gate clears** - Delegate to `tsh-code-reviewer` following `.cursor/skills/commands/tsh-review/SKILL.md` only after the UI verification gate passes or is explicitly escalated per item.
