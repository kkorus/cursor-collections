...
7. **Handle `[REUSE]` UI verification as a per-item loop:**
   ...
   - **Credential selection is bound to a verified origin.** Record the origin (scheme, host, port) of the pinned user-confirmed dev server URL once per session and forward it to `tsh-ui-capture-worker` as the session's **authorized origin**. The worker must compare the current page origin *after all redirects* against it before deriving any field key, before reading any `TSH_UI_LOGIN_*` variable, and before any `fill`. A mismatch is a blocker the worker escalates with the observed origin instead of filling anything; you then ask the user to authorize that origin for the session, and only that explicit authorization permits a cross-origin login page — which is how SSO, MFA, and captcha screens hosted on another origin remain reachable rather than being refused. Never authorize an origin on the user's behalf and never instruct the worker to skip the check. The worker may read only the variables it requested for the form on screen at an authorized origin, never any other `TSH_UI_LOGIN_*` variable: the `TSH_UI_LOGIN_` prefix is a naming convention, not an allowlist, so the origin binding plus that request-scoping is what constrains which secret can reach a page.
   ...
   - The **LOW-confidence pause** defined in `tsh-implement-ui` is non-terminal in the same way `VERIFICATION NOT RUN` is: when a FAIL report carries LOW confidence, the loop stops and asks the user before any fix is delegated. It is not an iteration, it is not `VERIFICATION NOT RUN`, and it must never be recorded as `ESCALATED` — no blocker has been acknowledged by the user. It cannot close the item, cannot mark it PASSED or ESCALATED, and cannot open code review; the item stays open until the user answers and the verify-fix loop resumes with their direction.
