...
2. **Load required skills** (MUST complete before any analysis):
   ...
   - **Always load** the `tsh-technical-context-discovering` skill — follow its process to identify existing IaC patterns, tagging conventions, and project context.
   ...
   - **Always load** the `tsh-optimizing-cloud-cost` skill, including its `references/tagging-standards.md` — use it to establish the "Core 5" Mandatory Tags, Pricing Model Decision table, Storage Tiering Decision table, Process steps, Checklist, and Anti-Patterns.
   - **Conditionally load** the `tsh-implementing-terraform-modules` skill — load when Terraform files (`*.tf`, `*.tfvars`, `terragrunt.hcl`) are found. Use it as the reference standard for module structure, naming conventions, provider pinning, and AWS-specific module patterns when evaluating IaC quality and generating Terraform code.
   - **Conditionally load** the `tsh-managing-secrets` skill — load when the audit scope includes security (which is always for a full audit). Use its Security Checklist, Anti-Patterns, and Cloud-Native Detection patterns when evaluating secrets management, KMS keys, and credential handling.
   - **Degraded mode — Core 5 tag names only.** If `tsh-optimizing-cloud-cost` and its `references/tagging-standards.md` cannot be located, the audit may fall back to the hardcoded Core 5 Mandatory Tag names `CostCenter`, `Environment`, `Service`, `Owner`, `DataClass` — but only when **all three** of these hold:
     ...
     2. **The substitution is announced** — the report this command emits states, in its Executive Summary, that the tagging-standards skill could not be located and that the Core 5 tag-name defaults were substituted for it.
     3. **Tag names only** — the defaults supply the five names and nothing else. **No cost finding, no security finding, and no remediation recommendation may be issued from them.** The thresholds, decision tables, Checklist, and Anti-Patterns that justify a finding live in the skill that was not loaded, so the Tag Compliance section reports name presence or absence only, and the Optimization Opportunities and Critical Security Findings sections derive nothing from this fallback.
