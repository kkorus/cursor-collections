...
2. **Validate the plan** — After the architect produces or updates the `.plan.md`, invoke the `tsh-architect-reviewer` agent skill (`.cursor/skills/agents/tsh-architect-reviewer/SKILL.md`) to validate the plan's correctness, feasibility, and simplicity. If REVISIONS NEEDED with BLOCKERs → send report back to architect, re-validate (max 3 iterations, then escalate). If plan already approved and unchanged → skip. Once APPROVED, present plan and review summary to user for implementation confirmation.
