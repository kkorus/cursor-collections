...
2. **Load required skills** (MUST complete before any analysis):
   Every skill below is referenced by name, not by path. Resolve each name to its skill file with the `tsh-resolving-skill-references` resolution order, read the file, and — except where a degraded mode is explicitly permitted below — stop and ask the user if a name cannot be resolved.
   - **Always load** the `tsh-technical-context-discovering` skill — follow its process to identify existing IaC patterns, labeling conventions, and project context.
   - **Always load** the `tsh-codebase-analysing` skill — even if no IaC files are found initially, this skill's Step 7 (infrastructure code) helps identify IaC in unexpected locations, and Step 2 (dependencies) reveals infrastructure-related dependencies in any project.
   - **Always load** the `tsh-optimizing-cloud-cost` skill, including its `references/tagging-standards.md` — use its GCP columns in the Pricing Model Decision table, Storage Tiering Decision table, Process steps, Checklist, and Anti-Patterns. Adapt the "Core 5" Mandatory Tags for GCP as **labels**: `cost_center`, `environment`, `service`, `owner`, `data_class` (lowercase with underscores — GCP label format).
   - **Conditionally load** the `tsh-designing-multi-cloud-architecture` skill — load when evaluating GCP service alternatives or comparing GCP services against each other. Use its GCP service comparison tables for compute, storage, database, and networking decisions.
   - **Conditionally load** the `tsh-implementing-terraform-modules` skill, including its `references/gcp-modules.md` — load when Terraform files (`*.tf`, `*.tfvars`, `terragrunt.hcl`) are found. Use it as the reference standard for module structure, naming conventions, provider pinning, and GCP-specific module patterns when evaluating IaC quality and generating Terraform code.
   - **Conditionally load** the `tsh-managing-secrets` skill — load when the audit scope includes security (which is always for a full audit). Use its Security Checklist, Anti-Patterns, and Cloud-Native Detection patterns when evaluating secrets management, Cloud KMS keys, and credential handling.
   - **Degraded mode — Core 5 label names only.** If `tsh-optimizing-cloud-cost` and its `references/tagging-standards.md` cannot be located, the audit may fall back to the hardcoded Core 5 Mandatory Label names `cost_center`, `environment`, `service`, `owner`, `data_class` — but only when **all three** of these hold:
     1. **Resolution is exhausted** — the full resolution order has been attempted for this skill and every step of it failed. The defaults must never fire on a first missed location.
     2. **The substitution is announced** — the report this command emits states, in its Executive Summary, that the tagging-standards skill could not be located and that the Core 5 label-name defaults were substituted for it.
     3. **Label names only** — the defaults supply the five names and nothing else. **No cost finding, no security finding, and no remediation recommendation may be issued from them.** The thresholds, decision tables, Checklist, and Anti-Patterns that justify a finding live in the skill that was not loaded, so the Label Compliance section reports name presence or absence only, and the Optimization Opportunities and Critical Security Findings sections derive nothing from this fallback.
   - **Fallback for the remaining skills**: If a skill file still cannot be located **after the resolution order has been exhausted**, log the failure, note the skill gap in the final report's Executive Summary, and proceed with the skills that did load. This fallback never fires on a first missed location.
   - **Exception — `tsh-managing-secrets` is a gate, not an optional input.** When the audit scope includes security (which is always for a full audit) and `tsh-managing-secrets` cannot be located after the resolution order has been exhausted, **stop and ask the user** instead of proceeding. Its Security Checklist governs the report's Critical Security Findings section; a security section produced without its governing skill is a gate that did not pass, not a gap worth noting. Either the user supplies the skill's location, or the audit is re-scoped to exclude security and the report says so.
