...
- **Credential and path handling — five items, and the reason this batch was prioritized ahead of everything deferred.** Two of the five put live credentials at risk of being executed or committed, one handed a secret to a page whose origin was never checked, one could move screenshots of the signed-in application out of ignored scratch space into tracked space, and one shipped a copy-paste example that carried two of those defects into any project that copied it. This collection ships instruction text, so a defect in the text *is* the defect — none of these is a product-code change.
- **The documented `.env` loader *executed* the file rather than parsing it.** It used `set -a` followed by `source .env`, and `source` runs the file as shell — so a command substitution or a function definition sitting in a credential file would execute with the worker's privileges. The documented threat model was disclosure only, which is why the hole existed. The file is now read as **data**, with only variables matching the `TSH_UI_LOGIN_` contract exported and everything else ignored. The reader's five limitations are written down rather than implied away — quotes are not stripped, multi-line values are unsupported, an `export ` prefix is skipped, no escape processing happens, and it needs `bash` — and a credential that needs any of them is pointed at the storage-state path instead of arriving silently truncated.
- **`.gitignore` did not cover `.env` at all**, while four files in the authentication chain instruct you to fill that exact file with live login credentials. The only env rule present matched `.env*.local`, which covers neither `.env` nor `.env.production`. Ignore rules now cover `.env`, `.env.*`, and the authenticated-session and storage-state artifacts the same documentation tells you to create. Verified file by file, and no tracked file became ignored as a side effect. Separately, the storage-state reference told you to add a pattern that matched **none** of the state filenames used on its own page; it now tells a consuming project to add rules that actually match the filenames it uses, and says outright not to rely on the old one.
- **Credential selection had no origin binding at all.** The environment variable name is derived from page-controlled form fields and the value is handed back to the page, with the login page reached by an unchecked redirect — so the skill's existing "only credentials the user provided **for this task**" rule had nothing to make it satisfiable. The origin of the URL you confirm is now recorded once per session as the **authorized origin**, and the page origin *after all redirects* is checked against it before any field key is derived, before any `TSH_UI_LOGIN_*` variable is read, and before anything is filled. A mismatch is raised to you as a blocker with the observed origin rather than silently refused, so a cross-origin SSO chooser, MFA challenge, or captcha stays reachable — but only on your explicit authorization for that session. The rule that makes this necessary is now stated in the text: `TSH_UI_LOGIN_` is a **naming convention, not an allowlist**, so it is the origin binding plus per-form scoping that constrains which secret can reach a page. Landed across the Playwright CLI skill, the UI verification workflow, the UI capture worker, and the orchestration workflow, with the field-key derivation order unchanged.
- **The verification artifact directory was built by string concatenation from an untrusted identifier**, with a documented caller override that bypassed the prefix entirely. The verification ID is substituted straight into a path and can reach a worker from a generated plan file, so it is now treated as a single path segment and **refused** — not sanitized, not rewritten — when it contains a slash, a backslash, a `..` segment, a leading `~`, an absolute path, or a newline. A caller-supplied verification root is honoured only if it still resolves inside the repository's `specifications/` tree after normalization, so an absolute path inside that tree stays valid and the capability is bounded rather than removed. It matters because `specifications/` is ignored by git: a root that escapes it relocates screenshots of the **authenticated** application into tracked, committable space.
- **A copy-paste login example hardcoded two literal credential values and wrote the authenticated session into the working directory.** That snippet was the only place in its file that filled a form field, and it passed a literal string at each of its two calls before saving session state to a bare relative filename resolved against whatever directory you happened to run from. Both calls now read from the `TSH_UI_LOGIN_*` contract the same skill already mandates, the session file is written outside the repository working tree, and a closing line points you at the corrected ignore guidance for your own project. The two literal credential values in that copy-paste login example are removed, and they are named nowhere on this page. This half stayed live even after the ignore fix above, because that page is guidance you copy into your own project and this repository's `.gitignore` does not travel with it.
- **Control flow that contradicted itself — five items,** each a pair of instructions in one lineage that could not both be followed. **`networkidle`** was banned outright by the end-to-end testing lineage while the UI capture path mandated it; both statements are now scoped instead of one winning, because they govern different mechanisms — authored test code has auto-waiting assertions that retry, while one-shot evidence capture across arbitrary pages has no assertion to retry and no page-independent readiness signal. The capture path keeps it as a **bounded, non-fatal best-effort**: an explicit timeout, the throw caught, a fall-through to the weaker bounded `load` state, and a printed degradation signal, since both waits are swallowed and the command succeeds either way. A stabilization timeout is now documented as neither a blocker nor an escalation — it does not produce `VERIFICATION NOT RUN` and does not consume an iteration of the verification budget. The wait-strategy reference also stops listing `networkidle` first, because first position read as the default. The **confidence gate** in the UI implementation loop was defined *after* the step that had already dispatched the fix; it is now read before dispatch, and a LOW-confidence pause is explicitly not an iteration, not `VERIFICATION NOT RUN`, and not `ESCALATED` — which the old "continue with next component or escalate" fallback would have authorized. The **"report first" and "ask first"** orderings in the UI reviewer were mutually exclusive; the report is now always returned and never withheld pending an answer, the question travels inside the report's blocker-resolution section, and invocation mode decides who it is put to — a direct user is asked, a calling orchestrator receives the identical report and owns the question. The DevOps agent's **default fallback selected a technology stack** when the Architect was unavailable, which is the exact decision three other lines in that file reserve for the Architect; it now names the failed condition, selects nothing, and asks. And the **spec-driven testing guide** called its scenario loop safe to parallelise because session names are unique, while two other lines in the same file forbid parallelism and name the shared seed session and single CLI session as the reason — unique session names bound name collisions, not shared state, so the outlier was corrected toward wording the file already carried twice.
- **Text that was factually wrong — three items.** The generated-test template imported its fixtures with a specifier that cannot resolve from the test path the same guide prescribes; it is corrected, and the rule now states the fixture file's location so the specifier is derivable rather than memorized. Four published pages described this repository's own Cursor artifacts as Copilot files and named an artifact type that does not exist here; they now name command skills and `.mdc` rules correctly. And the Figma reference image path was stated two incompatible ways — the per-iteration directory on the published pages against the shared verification root in the skill that actually writes the file; the pages now match the skill.

### Changed

- **The naming rule was corrected rather than the tree renamed.** The rule claimed a `tsh-` prefix on "all artifact filenames" while all five of its own bullets prefix the *directory* and leave the filename bare, and many tracked files under `.cursor/` violate it as literally written. The rule was fixed instead of the tree: the prefix applies to skill **directory** names in all four layers and to rule **filenames**, and not to the literal `SKILL.md` (fixed by Cursor), not to in-skill support files (`references/`, `assets/`, `examples/`, `*.template.md`, `*.example.md`), and not to config files. A new named-exceptions section lists exactly two and states that a third requires editing the rule: **`playwright-cli`, recorded as a vendored third-party skill kept at its upstream name so upstream syncs stay diffable** — covering its directory, its frontmatter `name`, and its unprefixed reference files — and the repository-level instructions file, whose name is fixed by convention. **Nothing was renamed.** The rule ships in two copies and both changed together and remain byte-identical, and the repository instructions' own naming section was brought into line and now points at the exceptions.

### Notes

- **Disposition of all 43 findings CodeRabbit filed on PR #1, stated at both count levels — because merging the two levels is what made the plan behind this delivery wrong once already.** A **finding** is one CodeRabbit inline comment; a **plan item** is one unit of work. They are not the same number and not interchangeable. **17 findings were accepted**, closed by **14 plan items** — the two differ by three because three items each close two findings: one closes the same `networkidle` split both where it was mandated and where it was presented as the default wait strategy; one closes both the report-vs-ask ordering and the subagent clarification route, since a single report-with-embedded-question settles both; and one closes both the self-contradicting prefix rule and the request to rename `playwright-cli` — that second one closed with an alternate remedy, the named vendored-third-party exception, rather than the rename it asked for, and recorded as accepted-with-alternate-remedy so the count is auditable in both directions. **7 were invalid**, each with its premise falsified by the very file the finding cited. **2 were refused** by explicit user decision. **17 were deferred** — real, but not this delivery. **17 + 7 + 2 + 17 = 43.** The 43 comprise 29 Major, 13 Minor, and 1 Trivial. The per-finding rationale lives in the implementation plan rather than being reproduced here one entry at a time; that plan is not published with the repository, so **these four numbers are the only public record of how the 43 were dispatched** — which is why they are copied from the plan's canonical disposition table rather than recounted while writing this entry.
- **What was verified, and what was not.** The documentation-site build succeeds. The collection still counts 25 agents, 17 commands, 39 workflow skills, and 12 internal skills (93 total) — and because that count inspects directories without reading their names, it guards against additions and removals only, so the no-rename claim above rests on the change list instead. The `disable-model-invocation` inventory is unchanged at **43 keys over the glob `.cursor/skills/*/*/SKILL.md` and 44 over the glob `.cursor/**/*.md`**, both by frontmatter-block extraction with the pattern `^disable-model-invocation:`; the same extraction yields zero for each of `model:`, `tools:`, and `agents:`. **There is no test framework, no end-to-end suite, no linter, and no CI in this repository:** the documentation-site build is the only executable gate, so no tests were run and none are claimed — everything else is a deterministic text or tree assertion. **No credential value, real or example, is introduced by this delivery, and none is named in either changelog entry.**

## 2026-09-05 (record correction — upstream is reachable, so two published notes and one closed finding were wrong)

### Added

- The task-to-owner routing table now covers Cursor customization artifacts, closing the item the entry below published as **"Open, not fixed"**. A bounded change to a single customization artifact routes to the Cursor Engineer as the DEFAULT; work spanning several artifacts, or needing research → create → review phases, routes to the Cursor Orchestrator as the EXCEPTION. Both rows name `tsh-creating-agents`, `tsh-creating-skills`, `tsh-creating-commands`, or `tsh-creating-rules` matched to the artifact type, and both describe the artifacts by type — agent, workflow, command, and internal skill files, and project rule files — rather than by directory path, because the table is delegation text. The table goes from 12 to 14 rows and the two `[REUSE]` rows are still last. **This is a deliberate fork divergence, and it is recorded as one so the next upstream sync reads it as intentional rather than as drift to reconcile away:** upstream's own routing table has no equivalent row — read at upstream `main`, it carries the same 12 rows and never mentions customization at all — so the gap is inherited, and closing it is a fork decision rather than a fidelity fix.
- The Cursor Orchestrator gained an escalation pointer: one sentence stating that work needing an implementation plan, a plan review, or Human approval is routed to the implementation-delivery route through `tsh-engineering-manager` rather than delegated from there. This is the **`option (iii)`** branch of the PR #77 roster question, chosen by the user at the plan-authoring approval gate; the roster note below records what was declined and why.

### Fixed

- Two surfaces still asserted a model binding their source does not have — the two residuals of PR #77's "a prompt routes to an agent; model selection is inferred from that agent" reframing **that the sweep's pattern could reach**, not the last such residuals: an exhaustive enumeration of every line in the collection, the documentation, and the README that uses the word "model", hand-classified one line at a time, puts the family at three surviving sites, and the bullet below names all three. The PR #77 port commit applied that reframing to some sites and not others. The `tsh-create-custom-command` command skill instructed every future command author that a command skill "must specify an agent routing and model preference in its frontmatter", and **both halves were false**: frontmatter across all 17 command skills carries exactly `name`, `description`, and `disable-model-invocation`, so neither a model preference nor an agent routing lives there — and the same file already stated the opposite about routing, that every command must reference an agent in its **body**. The Engineering Manager agent page (`website/docs/agents/engineering-manager.md`) said `/tsh-implement` routes to the agent "using the same shared model array", thirteen lines after that page had already published the ratified statement that model selection is a session-level concern in Cursor, handled per worker at delegation time and not bound by the artifact — so the page contradicted itself. Both now carry the ratified wording, and the repository-wide sweep for those two phrases goes from two hits to zero.
- **That zero is true and incomplete, and the shortfall is disclosed rather than quietly closed — but the first disclosure of it was itself built the wrong way, and that is the finding.** **Three** surfaces in this family survive, not one — the family being a surface that asserts a model binding its own source disclaims. Two of the three are semantically identical to each other, the *same taxonomy bullet* asserting the same false artifact–model coupling in different words: the Cursor Engineer agent skill (`.cursor/skills/agents/tsh-cursor-engineer/SKILL.md:47`), which describes a command skill as routing work "to a specific agent and model", and the Cursor Orchestrator agent skill (`.cursor/skills/agents/tsh-cursor-orchestrator/SKILL.md:77`), which says the same skill "routes to agent + model". The third is the same defect in a different construct — where those two bind an *artifact* to a model, it binds *workers* to models, and that difference is exactly why a pattern built for the other two could not see it. It is the Analyze Materials command skill (`.cursor/skills/commands/tsh-analyze-materials/SKILL.md:62`), which tells every reader that "The BA orchestrator may route transcript cleanup, analysis, extraction, quality review, and formatting to internal model-specialized BA workers" — a model binding the agent it describes disclaims in terms, because the Business Analyst agent skill (`.cursor/skills/agents/tsh-business-analyst/SKILL.md:20`) reads "Model selection is handled at the Cursor session level per worker — do not hardcode model names here." The sweep's two-phrase pattern sees **none of the three**, reporting zero on each of those files, so the zero that certifies the fix above is blind to all of them. All three are pre-existing and verified as such against the previous commit, so **none is a regression of this round**; all three are deliberately left in place, the same disposition for each, because fixing any of them would widen a scope that was already approved. All three are recorded as out of scope in the plan, the latter two added after the independent review found them — and the plan numbers them the **third, fourth, and fifth** model-coupling sites of the round because it counts the two this round *fixed* as well, while this entry counts only the three left unrepaired. That plan is not published with the repository, so this page and the canonical changelog are the only record of the three that ships. **The original disclosure named only the first because that was the instance someone stumbled over while working, not the output of a sweep derived from the concept** — which is why it missed the second, sitting seventy lines below the sentence this round inserted into that very file. The sweep that replaced it is **not** generated from the concept either, and saying so is the point: it is the same instance-derived pattern widened to the wordings by then known to be defective — four alternatives for four known phrasings — which is precisely why it returns exactly four hits across `.cursor`, the documentation, and the README and no more: the first two sites above, the now-corrected Engineering Manager page, and one legitimate non-target in the FAQ ("avoid needless agent and model switching" — advice about your own model picker, not a claim that an artifact declares a model). It reports zero on the Analyze Materials skill, and the mechanism is worth stating precisely, because the obvious diagnosis is the wrong one: the pattern's length bound between "routes to" and the word "model" is not what fails. The pattern requires "route" or "routes" followed immediately by "to", or by "work to", and on that line **74 characters** of list sit between them — " transcript cleanup, analysis, extraction, quality review, and formatting " — so that alternative never engages at all; "model-specialized" then matches none of the other three alternatives either. **What did find it was an enumeration, and that enumeration is the only check here generated from the concept rather than from known instances, because it does no phrase matching at all:** every line under `.cursor`, the documentation, the README, and the root code-quality report that contains the word "model" or "models", excluding the `disable-model-invocation` frontmatter key, comes to **130** lines, and all 130 were then classified by hand; three are of this family. **Its limits belong in the same breath, or one overclaim has merely replaced another.** The enumeration is exhaustive over that one word in those four paths, so a coupling claim that never uses the word "model" would still escape it; and the classification of the 130 is judgement, not measurement. So "three" is the count that survived a hand pass over every line containing the word — a materially stronger basis than a pattern's zero, but not a machine-checkable invariant. **So this is the note below's own lesson recurring inside the bullet that teaches it, three times over: once in the sweep that certifies the fix, once in the first disclosure that named a single stumbled-over site, and once in this bullet's own earlier claim that its widened pattern had been generated from the concept when it had not — and each time it was the independent review of this entry that caught it, not the round that wrote the bullet.**

### Notes

- **Upstream is reachable from this machine, which falsifies the premise both notes in the entry below were bounded by.** The `gh` CLI reaches `TheSoftwareHouse/copilot-collections` through its own authenticated session — pull-request metadata and full diffs both return for PRs #76, #77, and #79 — and **no git remote was added**; `origin` still points only at this fork. The three checks the old premise rested on were each individually correct, and all three still hold: `TSH_COPILOT_COLLECTIONS` is unset, there is no sibling `copilot-collections` clone beside the workspace, and the only configured remote is `origin`. **The defect was the inference, not the checks** — the capability's absence was concluded from the absence of three specific mechanisms for it, and none of those three is the mechanism that actually works.
- **The PR #77 roster change: the disposition survives, its stated basis does not.** Diffed against upstream, the change is a single frontmatter line on upstream's own orchestrator agent — an `agents:` list of four Copilot workers gaining `tsh-plan-reviewer` and `tsh-architect`. `agents:` is a Copilot frontmatter key that **no artifact in this repository carries** — it appears in zero Markdown files under `.cursor`, as do `model:`, `tools:`, and `user-invokable:` — so non-applicability rests on **artifact shape**: there is no key here to port it into. That claim is deliberately about *this repository's artifacts* and not about Cursor's schema, which was never measured here: the collection's own agent-authoring guidance documents `agents` as an optional frontmatter field, an array of agent names available as subagents. Whether that surface is accurate is a separate, pre-existing question and is not decided here — the disposition holds either way, because nothing here carries the key to port into. The note below justified it instead with a prose-roster *completeness* check, paraphrased here rather than quoted because an earlier draft of this bullet put quotation marks around a paraphrase: it verified that all four workers exist as agent directories and that the only other `cursor-*` agent directory is the orchestrator itself, so no fifth worker is missing from the list. That answers a different question than the change asks — the change adds two agents from outside the `cursor-*` family, so a check looking for a missing fifth `cursor-*` worker could not have found the defect. On completeness grounds the roster was in fact *not* complete relative to upstream's: neither `tsh-plan-reviewer` nor `tsh-architect` appears in that file, before this round or after it. The *semantics* were a real architectural question, settled separately as **`option (iii)`** — decline the semantic port, add an escalation pointer — because the Architect and the Plan Reviewer are gate-bearing rather than generic workers, and the Cursor Orchestrator carries no Human-approval precondition and no `Plan Revision` awareness, so a delegation edge would give the approval gate a second, ungoverned entrance. The strongest argument against that choice is preserved in the plan rather than dropped.
- **The commit-order claim is false on verification, and three surfaces said three different things about it.** Upstream merge timestamps: #76 `2026-07-28T10:45:05Z`, #77 `2026-07-29T12:37:27Z`, #79 `2026-08-05T07:49:45Z`. Upstream merge order is therefore **#76 → #77 → #79** — precisely the ascending numbering the landed note goes out of its way to rule out. The port landed #76 → #79 → #77, so it did **not** follow upstream merge order, and it broke `tsh-migrating-copilot-to-cursor`'s rule to port PRs in merge order, one commit per PR, in the order they merged so dependencies resolve. The three surfaces differ, and flattening them into a single error would misstate the record. The **plan** is the origin of the assertion, claiming "upstream merge order" outright. **The canonical changelog's note is the mildest form and deserves credit for flagging its own basis** — it says "**The plan asserts that this is** upstream merge order" and then states outright that this is the plan's assertion and not an independent check, so it reached a wrong conclusion while being honest that it had not verified one. **The round's final commit message is the worst form:** it states flatly that the commit order was both the planned phase order and the order the migration skill mandates, and **closes the finding as a not-defect with no hedge at all**. Its first half is true — that was the planned phase order. Its second half is false. **Nothing is rewritten:** no commit is amended, so that message is immutable, which is itself the argument for the correction living in a new changelog entry — the only surface still reachable. **The mechanism is the transferable part.** The false unreachability premise *shielded* this claim: because upstream had been declared unreadable, "this is merge order" was settled by reading the plan's own assertion instead of running the check. And the premise contradicted an explicit instruction in the very skill the port was executing, two lines above the rule it caused the port to break: "**Fetch the diff, don't guess:** `gh pr diff <n> --repo <upstream>`. Read the whole diff before touching anything."
- **The reusable lesson, and it now has three instances rather than two.** In each, a check was built as a *pattern derived from an assumption about the shape of the defect* rather than run against the thing itself, and its clean result was then read as proof. (1) The port's criterion that this page must contain no `## 2026-09-04` entry, and the naive mention-count that replaced it, whose zero was reachable only by deleting the gap notes the criterion's own intent required. (2) The `disable-model-invocation` figure of "43", stated without the glob it measures or the pattern it matches, so it could not distinguish a real invariant from three different higher numbers. (3) **New, and found while executing the very task whose definition of done it evades:** the two-phrase sweep that certifies this round's model-coupling fix cannot see *any* of the three surviving sites named under **Fixed** above, all of which make the same false claim in different words — and disclosing that blindness reproduced the lesson twice more before it settled. The first attempt named only one site, because it too was written from a stumbled-over instance rather than from the concept. The second widened the pattern to the wordings then known and claimed the result had been generated from the concept, which it had not: a wider instance-derived pattern still cannot see a wording nobody has looked for, and it duly reported zero on the third site, the Analyze Materials command skill. Both misses were caught by the independent review of this entry, not by the round that wrote the bullet. The false unreachability premise is the same shape one level up — three mechanisms were probed, and the capability was inferred absent from their absence. The lesson is none of the individual corrections: a check meant to *discover* has to be generated from the concept rather than from the instances already known to be defective, and a zero from such a pattern bounds nothing on its own.
- **Process deviation, disclosed rather than omitted.** The Implementation Discussion Boundary states that implementation begins only in a new discussion. This round executed in the plan-authoring discussion, on the user's explicit re-invocation after the boundary was raised and the user overrode it. **The sentence that was actually crossed is stated here rather than left out:** in the authoring discussion, after a current-revision recorded `APPROVED`, the manager reports the exact plan path, current `Plan Revision`, persisted `Decision Timestamp`, and review path when present, names implementation as the next step, and "MUST NOT perform any file-changing delegation there". That is the prohibition this round crossed. Crossing it did not invalidate the approval, and the ratified source says why on its own terms: the boundary "is a lifecycle stop, never an approval-validity criterion", and it "does not detect or enforce any editor or chat conversation identifier". The persisted record still satisfies the validity predicate unchanged — `APPROVED`, an `Approved Revision` equal to the current `Plan Revision`, and a `Decision Timestamp` that is valid ISO 8601 UTC ending in `Z`. It is nonetheless a deviation from the canonical flow, so it is recorded here rather than left silent.
- **Independent code review of this round returned PASS with no blocking findings — and how the PASS was reached matters more than the verdict.** Six earlier findings from that review were closed **by correction, not by justification**, and the reviewer re-verified each against the tree rather than against this round's own summary: an undisclosed fourth model-coupling site; an exhaustivity overclaim; a global schema claim drawn from a local measurement; a non-sequitur that read the skill-counting script's output as evidence about files it says nothing about; a paraphrase presented as a quotation; and a heading on this page that understated the entry's scope. A seventh, selective quotation in the process-deviation bullet above, was closed the same way. The two minor items the review raised — the mislabelled sweep and the third surviving site — were **folded into the Fixed bullets above rather than deferred**, because both are about this entry's own accuracy. **One detail is independent corroboration rather than self-congratulation, so it is recorded:** the reviewer's own first attempt to confirm the four non-changelog files unchanged filtered the diff for added and removed lines with a pattern that **silently dropped** the Engineering Manager page's bullet replacement, because the removed line is a Markdown bullet and so begins with two dashes. That is the exact trap this plan's own definition of done documents and bans, reproducing itself inside the review tooling built to check the work; the reviewer caught it and re-read the change instead. **There is no test framework, no e2e suite, no linter, and no CI in this repository:** no tests were run and none are claimed. The gates are the text assertions, the skill-counting script, and the documentation-site build, which passed.
- No skill artifact was added or removed — the collection still counts 25 agents, 17 commands, 39 workflow skills, and 12 internal skills (93 total). The `disable-model-invocation` inventory is unchanged at **43 keys over the glob `.cursor/skills/*/*/SKILL.md` and 44 over the glob `.cursor/**/*.md`**, both measured by frontmatter-block extraction with the pattern `^disable-model-invocation:` — stated with both the glob and the pattern, because a bare "43" is exactly the imprecision the entry below left open. Nothing was committed, staged, or pushed by this round, and no remote was added.

## 2026-09-05 (follow-up — the findings left open after the #76/#79/#77 port)

### Fixed

- The Implementation Discussion Boundary is now published on all five workflow flow pages — it was on the Architect and Engineering Manager pages and on **none** of the flow pages, so the pages you follow to run the workflow never said where implementation starts. The Workflow Overview, Standard Flow, E2E Testing Flow, Frontend Flow, and UI Verification Flow pages now each state the same four-part contract, reusing wording that is already published rather than inventing new phrasing — the report-duty clause tracks the Engineering Manager agent page, while the load-bearing **lifecycle stop, not an approval-validity criterion** appears on neither agent page and is taken from the ratified orchestration skill itself: recording plan-authoring Human approval ends the **authoring discussion** — the discussion in which the plan was authored, reviewed, and approved; the Engineering Manager there reports the exact plan path, the current `Plan Revision`, the persisted `Decision Timestamp`, and the review path when present, names implementation as the next step, and delegates no file change; you start a new discussion to implement, where the unchanged persisted record is reused without a duplicate approval gate; and the boundary is a **lifecycle stop, not an approval-validity criterion** — an invalid or missing record still fails closed, and a material revision still requires renewed Human approval.
- The boundary now also appears in each page's step sequence, not just its prose — the Standard Flow, Frontend Flow, and E2E Testing Flow pages gained a "Authoring discussion ends here — start a new discussion to implement" step, and the manager is now shown validating the record *in that new discussion*. The UI Verification Flow diagram gained an "Authoring discussion ends" node between the recorded approval and the manager's validation, plus a new never-optional rule that implementation runs in a new discussion and never in the authoring one. The Workflow Overview page additionally states the one exclusion: when reviewer readiness rests on the initial-plan low-risk automated-review exemption, the Architect's plan-authoring gate never ran, so there is no authoring discussion to close and the manager's gate is the only user-facing gate.
- The Prompts Overview page published a wrong approval model and a route that no longer exists, in a single sentence — it attributed the routine approval gate to the Engineering Manager and referred to "either flow", the last reference anywhere in the documentation to the second implementation route this port abolished. The port introduced both; neither was pre-existing staleness. That sentence now states the recorded-approval model: approval must already be recorded before the first file-changing delegation, the Architect normally records it at its own plan-authoring gate (`Approve plan`, `I have comments`), and the Engineering Manager validates and reuses a valid record, presenting its own `Approve current plan` / `Request changes` / `Stop` gate only as fail-closed recovery. It names no route count at all, and still says that automated Reviewer approval is readiness evidence only rather than permission to implement.

### Changed

- The orchestration skill's preservation-coverage table gained one clarifying sentence: checklist items 5-8 covered the delegation-and-review branch of the abolished second implementation route, the numbering gap is deliberate, and the surviving numbers are not renumbered so they stay traceable to the plan's original preserved-branch identifiers. **The finding behind this — that the table no longer covers items 5-8 — was miscalibrated, and it is closed as not a defect rather than fixed.** The pre-port table carried a row for exactly that branch, and this port abolished the route it named, so deleting the row was correct and no coverage is missing. Renumbering the surviving identifiers would have destroyed the traceability the table exists to provide, so the gap was documented instead. Only the sentence was added.

### Notes

- **Two** pages still published the single-gate model after the earlier correction fixed the other four, and the reason both were missed is the round's most transferable lesson. The E2E Testing Flow page kept its single-gate sentence saying the Engineering Manager requires Human approval of the current plan revision, and named the Architect's gate nowhere — so the boundary prose later layered on top of it used the term "plan-authoring Human approval" with no owner on the page. It now states the same model as its four siblings, and its approval step is attributed to the Architect's gate. The Prompts Overview page carried **two** defects in one sentence: the same single-gate attribution, plus the phrase "in either flow" — the last surviving reference anywhere in the documentation to the second implementation route this port abolished. Both were written by the port itself rather than inherited. That sentence now attributes the gate as the flow pages do and states no flow count at all. **The lesson is about verification design.** Two independently calibrated sweeps both reported clean on that one sentence: the two-gate sweep matched the literal phrase "must obtain Human approval" while the page reads "requires Human approval", and the abolished-route sweep matched the literal name of the removed flow while the page said "either flow". Each pattern had been built from the sites already known to be defective, so neither could find a variant nobody had seen yet — and passing two independent checks made the sentence look doubly verified when in truth nothing had examined it. A sweep meant to *discover* has to be generated from the concept rather than from the known instances — here, every site that mentions Human approval at all, and every site that implies more than one route — with each hit read in context instead of a zero count being treated as proof.
- Commit order, disclosed now rather than rewritten — the port landed as **#76 → #79 → #77**. **The plan asserts that this is upstream merge order**, which `tsh-migrating-copilot-to-cursor` requires — but that is the plan's assertion, not an independent check, because upstream is unreachable on this host for the reasons in the roster note below. What *is* independently verifiable, and holds: the shipped commit sequence #76 → #79 → #77 matches the plan's Phase 1, Phase 2, and Phase 3 order exactly. It is **not** the ascending `#76, #77, #79` numbering suggested by the batch name and by the plan's Jira ID and Title rows; the plan's Description row states "upstream merge order" outright. Neither changelog stated the order or its rationale at the time, which is what is recorded here. The commits are already published, so the ordering itself is not being rewritten.
- The round produced **five commits, not the planned three**, and that stands unamended deliberately: the three port commits are followed by two documentation-fix commits that repaired regressions the port itself introduced. Amending them away would rewrite already-verified history, break the one-commit-per-upstream-PR fidelity the port commits exist to provide, and hide that the extra commits were needed because the port shipped regressions.
- The PR #77 roster change in the Cursor Orchestrator skill was neither ported nor recorded as non-applicable at the time. **Disposition: non-applicable — with a stated limitation, not a clean verification.** The limitation comes first, because it bounds the claim: the change **cannot be compared against upstream on this host**. `TSH_COPILOT_COLLECTIONS` is unset, there is no sibling `copilot-collections` clone beside the workspace, and the only configured git remote is this fork. What *was* verified locally: the roster delegates to exactly `tsh-cursor-researcher`, `tsh-cursor-artifact-creator`, `tsh-cursor-artifact-reviewer`, and `tsh-cursor-engineer`; all four exist in the collection; and the only other `cursor-*` agent is the orchestrator itself, the roster's own owner, so no fifth worker is missing. The roster is complete and accurate **for this repository**, and that is what makes the change non-applicable here.
- One of the port's own verification criteria was false, and **one** completed check rested on it. The criterion appears at two Definition-of-Done sites: one was ticked, and the other stays unticked on purpose because its stated command and expected value are untrue as written; a third site is ticked but carries the corrected criterion rather than the false one. It required this snapshot to contain no `## 2026-09-04` entry — unsatisfiable the moment the port wrote its own dated entries here, and the reason those headings had earlier been inverted to a non-native shape purely to make a substring check pass. The naive replacement, counting mentions of the skill-reference-resolution skill on this page and expecting zero, was **also** false: the observed count is four, and three of those four are the pre-existing-gap notes the criterion's own intent requires, so zero was reachable only by deleting the notes that were mandatory. The surviving intent is narrower and still holds: no entry may backfill or describe that earlier work. It is now a stated human check over the enumerated mention sites, with the count demoted to a drift tripwire. The transferable lesson: no occurrence count can express "no entry *describes* X", because the prose recording the gap necessarily *mentions* X.
- **Open, not fixed** — the `disable-model-invocation` count was stated imprecisely as "43" throughout this work. Precisely, it is 43 across the collection's `SKILL.md` files and 44 across all Markdown files under `.cursor`; the 44th carrier is the command authoring template in `tsh-creating-commands`, which is a template rather than a skill. File-level counts run higher, and *how much* higher depends on the pattern — which is why a criterion has to state its pattern as well as its glob: a line-anchored search for the frontmatter key reports 44 and 45 files across those same two globs, because the `tsh-creating-commands` skill page itself shows the key at the start of a line inside a fenced YAML example, and a plain search for the bare string reports 50 and 53, because several skills discuss the key in prose. Only frontmatter-block extraction expresses the invariant. Nothing drifted — the count is unchanged before and after the port — only the wording was imprecise, and a future criterion should state both the glob it measures and the pattern it matches.
- **Open, not fixed** — the task-to-owner routing table has no row for Cursor customization artifacts. The collection's own `SKILL.md` files are this repository's primary product, yet they match neither the LLM-prompts row nor the documentation row, and the Prompt Engineer explicitly excludes them and points at the Cursor Engineer. This port was routed as plan seams, which works but bypasses the specialist. Changing the ratified routing table is outside this round.
- **Open, not fixed** — the pinned dev-server URL is collected before the approval gate, that is, in the authoring discussion, while UI capture runs in the implementation discussion. The ratified source has the same ordering, with the URL request preceding the approval gate in the planning sequence, so it was left alone rather than inventing a re-confirmation rule the source does not state. Recorded as a latent question for a future round.
